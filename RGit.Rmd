...
## Associate Your SSH Key from `R` Studio with Github

To avoid problems down the road, you will need to associate your `R` Studio SSH key (see above screen shots) with your GitHub account.
...
Then, click on "View Public Key", and copy it:
```{r, echo=FALSE}
...
Then, run the code suggested by GitHub.  to


# Creating a Repository (Repo) in `R` Studio and Initializing It

Now that we have `Git` set up and a repository on Github, we need to create a repository in `R` Studio as well. To do so, go to:
