...
Now that we have `Git` set up, we need to create a repository in `R` Studio. To do so, go to:
...
# Initializing a Repository on GitHub and Linking it Locally

Unfortunately, doing everything locally on `R` Studio is not enough to link everything with GitHub. To get started, log on to GitHub. Then, click on "New" under the "Repositories" tab:
...
Since we created the repo on Github, we will use the second option, "…or push an existing repository from the command line". To do so, go to `Git Bash` (Windows users) or the Shell (Mac users), and run your code. In my case, it looks as follows:
